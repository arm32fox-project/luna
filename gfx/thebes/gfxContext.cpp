...
gfxContext::Mask(gfxASurface *surface, const gfxPoint& offset)
{
  ...
  if (mCairo) {
    ...
    // Since we're already passing the offset as a transform, we have to
    // temporarily set the device offset to 0,0 to avoid using it twice if
    // sourceSurf still has access to it.
