...
public class GoannaScreenOrientationListener {
    private static final String LOGTAG = "GoannaScreenOrientationListener";
    ...
    static class OrientationEventListenerImpl extends OrientationEventListener {
        ...
        public void onOrientationChanged(int aOrientation) {
            GoannaScreenOrientationListener.getInstance().updateScreenOrientation();
        }
    }

    static private GoannaScreenOrientationListener sInstance = null;
    ...
    // Whether the listener should notify Goanna that a change happened.
    ...
    private GoannaScreenOrientationListener() {
        mListener = new OrientationEventListenerImpl(GoannaAppShell.getContext());
    ...
    public static GoannaScreenOrientationListener getInstance() {
        if (sInstance == null) {
            sInstance = new GoannaScreenOrientationListener();
        }
    ...
    private void updateScreenOrientation() {
      ...
      if (context instanceof Activity) {
        ...
        if (mShouldNotify && mOrientation != previousOrientation) {
            GoannaAppShell.sendEventToGoanna(GoannaEvent.createScreenOrientationEvent(mOrientation));
        }
    }
	...
	if (GoannaAppShell.getContext() instanceof Activity)
	  ((Activity)GoannaAppShell.getContext()).setRequestedOrientation(orientation);
        updateScreenOrientation();
    }
    ...
    public void unlockScreenOrientation() {
      if (!(GoannaAppShell.getContext() instanceof Activity))
	return;
      if (((Activity)GoannaAppShell.getContext()).getRequestedOrientation() == mDefaultOrientation)
	return;

      ((Activity)GoannaAppShell.getContext()).setRequestedOrientation(mDefaultOrientation);
        updateScreenOrientation();
    }
}
