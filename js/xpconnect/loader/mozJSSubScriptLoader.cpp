...
struct LoadSubScriptOptions : OptionsBase {
    ...
    virtual bool Parse()
    {
        //k8: i know, this looks wrong and overcomplicated. i'll fix this later
        nsCString str;
        if (!ParseString("charset", str)) return false;
        // convert to unicode
        PRUnichar *stra = ToNewUnicode(str);
        if (stra && stra[0])
        {
            charset.Assign(stra);
            NS_Free(stra);
                                    ...
                                    JS::Value* retval)
{
    ...
    if (!JS_ValueToObject(cx, target, options.target.address()))
                                                 ...
                                                 JS::Value* retval)
{
    ...
    if (!script) {
        ...
        writeScript = !!script;
        if (options.ignoreCache) cache = nullptr; // do not write this script to cache
    }
